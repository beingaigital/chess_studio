	  ...
	  ui(new Ui::NewGameDialog)
{
	...
	// Initially hide difficulty group (will show when CPU player is selected)
...
{
	if (playerType(side) == CPU)
		return new EngineBuilder(preparedEngineConfiguration(side));
...
bool NewGameDialog::allowUndoEnabled() const
{
	return ui->m_gameSettings->allowUndoEnabled();
}

TimeControl NewGameDialog::timeControl(Chess::Side side) const
{
	return ui->m_gameSettings->timeControl(side);
}

GameAdjudicator NewGameDialog::gameAdjudicator() const
{
	return ui->m_gameSettings->adjudicator();
}
...
EngineConfiguration NewGameDialog::preparedEngineConfiguration(Chess::Side side) const
{
	...
	if (engineName.contains("Stockfish", Qt::CaseInsensitive))
	{
		...
		// This ensures Skill Level takes precedence
