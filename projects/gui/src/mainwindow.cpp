...
void MainWindow::createActions()
{
	...
	m_undoMoveAct = new QAction(tr("&Undo Last Move"), this);
	...
	m_aboutAct = new QAction(tr("&About Chess Studio..."), this);
	...
	connect(m_undoMoveAct, &QAction::triggered,
		this, &MainWindow::undoLastMove);
...
void MainWindow::createMenus()
{
	...
	m_gameMenu->addAction(m_undoMoveAct);
	m_gameMenu->addSeparator();
...
void MainWindow::addGame(ChessGame* game)
{
	...
	PendingGame pendingInfo;
	auto pendingIt = m_pendingGames.find(game);
	if (pendingIt != m_pendingGames.end())
	{
		pendingInfo = pendingIt.value();
		m_pendingGames.erase(pendingIt);
	}

	const bool replacing = (pendingInfo.replaceTabIndex >= 0
				&& pendingInfo.replaceTabIndex < m_tabs.size()
				&& tournament == nullptr);

	if (replacing)
	{
		startGameReplacement(m_tabs[pendingInfo.replaceTabIndex],
				     pendingInfo.replaceTabIndex,
				     game,
				     pendingInfo.setup);
	}
	else
...
void MainWindow::startGameReplacement(TabData& tab,
				      int index,
				      ChessGame* newGame,
				      const GameSetup& setup)
{
	PgnGame* oldPgn = tab.m_pgn;
	QPointer<ChessGame> oldGame = tab.m_game;

	tab = TabData(newGame, tab.m_tournament);
	tab.m_setup = setup;
	tab.m_finished = false;
	...
	if (!oldGame.isNull())
		oldGame->deleteLater();
	m_gamesPendingReplacement.remove(oldGame.data());
}

void MainWindow::promptSaveGameToDatabase(ChessGame* game)
{
	if (game == nullptr || game->pgn() == nullptr)
		return;
	if (game->pgn()->moves().isEmpty() && game->pgn()->result().isNone())
		return;

	QSettings settings;
	if (!settings.value("games/prompt_save_to_database", true).toBool())
		return;

	QMessageBox box(this);
	box.setWindowTitle(tr("Save Game"));
	box.setText(tr("Do you want to save this game to your PGN database?"));
	box.setInformativeText(tr("You can review it later from the Game Database."));
	QAbstractButton* saveBtn = box.addButton(tr("Save"), QMessageBox::AcceptRole);
	box.addButton(QMessageBox::Cancel);

	QCheckBox dontAsk(tr("Don't ask again"));
	box.setCheckBox(&dontAsk);

	box.exec();

	if (box.clickedButton() == saveBtn)
		appendGameToDatabase(game->pgn());

	if (box.checkBox()->isChecked())
		settings.setValue("games/prompt_save_to_database", false);
}

bool MainWindow::appendGameToDatabase(const PgnGame* pgn)
{
	if (pgn == nullptr)
		return false;

	QSettings settings;
	QString target = settings.value("games/default_database_file").toString();
	if (target.isEmpty())
	{
		target = QFileDialog::getSaveFileName(this,
				tr("Select PGN database"),
				QDir::homePath(),
				tr("PGN files (*.pgn)"));
		if (target.isEmpty())
			return false;
		settings.setValue("games/default_database_file", target);
	}

	if (!pgn->write(target))
	{
		QMessageBox::warning(this, tr("Save Game"),
				     tr("Unable to append the game to %1").arg(target));
		return false;
	}
...
void MainWindow::setCurrentGame(const TabData& gameData)
{
	...
	if (m_game != nullptr)
	{
		...
		disconnect(m_game, SIGNAL(moveMade(Chess::GenericMove, QString, QString)),
	...
	// Connect moveMade signal to update menus (for undo button state)
	if (m_game != nullptr)
	{
...
void MainWindow::newGame()
{
	...
	GameSetup setup;
	setup.allowUndo = dlg.allowUndoEnabled();
	setup.adjudicator = dlg.gameAdjudicator();
	for (int i = 0; i < 2; ++i)
	{
		auto side = Chess::Side::Type(i);
		if (dlg.playerType(side) == NewGameDialog::Human)
		{
			setup.players[i].kind = PlayerKind::Human;
			setup.players[i].humanName = CuteChessApplication::userName();
			setup.players[i].playAfterTimeout = QSettings().value(
				"games/human_can_play_after_timeout", true).toBool();
		}
		else
		{
			setup.players[i].kind = PlayerKind::Engine;
			setup.players[i].engineConfig = dlg.preparedEngineConfiguration(side);
		}
	}

	PendingGame pending;
	pending.setup = setup;
	pending.replaceTabIndex = -1;
	m_pendingGames.insert(game, pending);
...
PlayerBuilder* MainWindow::createBuilderFromSetup(const PlayerSetup& setup) const
{
	if (setup.kind == PlayerKind::Human)
		return new HumanBuilder(setup.humanName, setup.playAfterTimeout);
	return new EngineBuilder(setup.engineConfig);
}
...
void MainWindow::onGameStartFailed(ChessGame* game)
{
	...
	if (pendingIt != m_pendingGames.end())
	{
		if (pendingIt.value().replaceTabIndex >= 0
		&&  pendingIt.value().replaceTabIndex < m_tabs.size())
		{
			const auto& tab = m_tabs.at(pendingIt.value().replaceTabIndex);
			if (!tab.m_game.isNull())
				m_gamesPendingReplacement.remove(tab.m_game.data());
		}
		m_pendingGames.erase(pendingIt);
	}
	game->deleteLater();
...
{
	if (m_gamesPendingReplacement.contains(game))
	{
		m_gamesPendingReplacement.remove(game);
		return;
	}
...
void MainWindow::updateMenus()
{
	...
	m_undoMoveAct->setEnabled(canUndoCurrentGame());
}

bool MainWindow::canUndoCurrentGame() const
{
	...
	// Cannot undo in tournaments
	if (tab.m_tournament != nullptr)
		return false;
	
	// Check if undo is enabled for this game
	if (!tab.m_setup.allowUndo)
		return false;
	
	// Cannot undo if game is finished or game object is null
	if (tab.m_finished || tab.m_game.isNull())
		return false;
	
	// Cannot undo if there are no moves
	if (tab.m_pgn == nullptr || tab.m_pgn->moves().isEmpty())
		return false;
	
	// Cannot undo if game is being replaced
	if (m_gamesPendingReplacement.contains(tab.m_game.data()))
		return false;
...
{
	if (!canUndoCurrentGame())
		return;

	const int index = m_tabBar->currentIndex();
	TabData& tab = m_tabs[index];
	const auto moves = tab.m_pgn->moves();

	if (moves.isEmpty())
		return;

	auto truncated = new PgnGame(*tab.m_pgn);
	const int newCount = moves.count() - 1;
	truncated->truncate(newCount);
	if (truncated->moves().count() != newCount)
	{
		delete truncated;
		return;
	}

	QString variant = truncated->variant();
	if (variant.isEmpty() && !tab.m_game.isNull() && tab.m_game->board() != nullptr)
		variant = tab.m_game->board()->variant();

	std::unique_ptr<Chess::Board> board(Chess::BoardFactory::create(variant));
	if (!board)
	{
		delete truncated;
		QMessageBox::warning(this, tr("Undo failed"),
				     tr("Unable to rebuild the board for the current variant."));
		return;
	}

	auto newGame = new ChessGame(board.release(), truncated);
	newGame->setAdjudicator(tab.m_setup.adjudicator);
	...
	for (int i = 0; i < 2; ++i)
	{
		...
		if (!tab.m_game.isNull())
		{
			ChessPlayer* player = tab.m_game->player(side);
			if (player != nullptr && player->timeControl() != nullptr)
			{
				TimeControl tc = *player->timeControl();
				newGame->setTimeControl(tc, side);
			}
		}
	}

	if (!newGame->setMoves(*truncated))
	{
		delete newGame;
		QMessageBox::warning(this, tr("Undo failed"),
				     tr("Unable to rebuild the game state for this position."));
		return;
	}

	Chess::Side toMove = newGame->board()->sideToMove();
	if (!toMove.isNull() && tab.m_setup.players[toMove].kind == PlayerKind::Human)
		newGame->pause();

	PlayerBuilder* builders[2] = {
		createBuilderFromSetup(tab.m_setup.players[Chess::Side::White]),
		createBuilderFromSetup(tab.m_setup.players[Chess::Side::Black])
	};

	PendingGame pending;
	pending.setup = tab.m_setup;
	pending.replaceTabIndex = index;
	m_pendingGames.insert(newGame, pending);

	connect(newGame, SIGNAL(initialized(ChessGame*)),
		this, SLOT(addGame(ChessGame*)));
	connect(newGame, SIGNAL(startFailed(ChessGame*)),
		this, SLOT(onGameStartFailed(ChessGame*)));

	CuteChessApplication::instance()->gameManager()->newGame(newGame,
		builders[Chess::Side::White], builders[Chess::Side::Black]);

	if (!tab.m_game.isNull())
	{
		m_gamesPendingReplacement.insert(tab.m_game.data());
		QMetaObject::invokeMethod(tab.m_game, "stop", Qt::QueuedConnection);
	}
...
{
	QString html;
	html += "<h3>" + QString("Chess Studio %1")
	...
	html += "<p>" + tr("Copyright 2008-2020 "
			   "Chess Studio contributors") + "</p>";
	html += "<p>" + tr("This is free software; see the source for copying "
			   "conditions. There is NO warranty; not even for "
			   "MERCHANTABILITY or FITNESS FOR A PARTICULAR "
			   "PURPOSE.") + "</p>";
	html += "<a href=\"https://github.com/biaowenhuang/chess-studio\">github.com/biaowenhuang/chess-studio</a><br>";

	QMessageBox::about(this, tr("About Chess Studio"), html);
}
