	  ...
	  m_isValid(true)
{
	...
	connect(ui->m_setupPositionBtn, &QPushButton::clicked,
		this, &GameSettingsWidget::openPositionSetupDialog);
...
bool GameSettingsWidget::allowUndoEnabled() const
{
	return ui->m_allowUndoCheck->isChecked();
}
...
void GameSettingsWidget::readSettings()
{
	...
	ui->m_allowUndoCheck->setChecked(s.value("allow_undo").toBool());
...
void GameSettingsWidget::enableSettingsUpdates()
{
	...
	connect(ui->m_allowUndoCheck, &QCheckBox::toggled, [=](bool checked)
	{
		QSettings().setValue("games/allow_undo", checked);
	});
}
...
void GameSettingsWidget::openPositionSetupDialog()
{
	PositionSetupDialog dlg(this);
	dlg.setFen(ui->m_fenEdit->text());

	if (dlg.exec() == QDialog::Accepted)
		ui->m_fenEdit->setText(dlg.fen());
}
