...
QString locateStockfishBinary()
{
	const QString envOverride = qEnvironmentVariable("CUTECHESS_STOCKFISH_PATH");
	if (!envOverride.isEmpty())
	{
		const QString resolved = resolveExecutable(envOverride);
		if (!resolved.isEmpty())
			return resolved;
	}

#ifdef Q_OS_WIN
	const QString executableName = QStringLiteral("stockfish.exe");
#else
	const QString executableName = QStringLiteral("stockfish");
#endif

	QStringList candidates;
	const QDir appDir(QCoreApplication::applicationDirPath());
	candidates << appDir.absoluteFilePath(executableName);
	candidates << appDir.absoluteFilePath(QStringLiteral("stockfish"));
	candidates << appDir.absoluteFilePath(QStringLiteral("../Resources/stockfish"));
	candidates << appDir.absoluteFilePath(QStringLiteral("../Resources/") + executableName);
	candidates << appDir.absoluteFilePath(QStringLiteral("../stockfish/") + executableName);
	candidates << appDir.absoluteFilePath(QStringLiteral("../../stockfish/") + executableName);
	candidates << appDir.absoluteFilePath(QStringLiteral("../../stockfish/src/stockfish"));
	candidates << appDir.absoluteFilePath(QStringLiteral("../../stockfish/stockfish-macos-m1-apple-silicon"));
	candidates << appDir.absoluteFilePath(QStringLiteral("stockfish/stockfish-macos-m1-apple-silicon"));
...
QList<EngineConfiguration> builtinEngineConfigurations()
{
	QList<EngineConfiguration> configs;

	const QString stockfishPath = locateStockfishBinary();
	if (!stockfishPath.isEmpty())
	{
		EngineConfiguration stockfish(QStringLiteral("Stockfish"),
			stockfishPath,
			QStringLiteral("uci"));
		stockfish.setWorkingDirectory(QFileInfo(stockfishPath).absolutePath());
		stockfish.setPondering(true);
		configs << stockfish;
	}
...
void ensureBuiltinEngines(EngineManager* manager, const QString& configFile)
{
	if (manager == nullptr)
		return;

	manager->loadEngines(configFile);

	const auto defaults = builtinEngineConfigurations();
	if (defaults.isEmpty())
		return;

	auto names = manager->engineNames();
	bool changed = false;
	for (const auto& engine : defaults)
	{
		if (names.contains(engine.name()))
			continue;
	...
	if (changed)
		manager->saveEngines(configFile);
}
	  ...
	  m_initialWindowCreated(false)
{
	...
	// Load the engines
	const QString engineConfigFile =
		configPath() + QLatin1String("/engines.json");
	ensureBuiltinEngines(engineManager(), engineConfigFile);
